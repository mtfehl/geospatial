...
# for larger datasets in spData package
...
# Multilines: use a LIST of matrices (each with each lines' coordinates):
...
sf.cities # watch out CRS (projection)! # need to specify:
# correct version:
sf.cities <- st_as_sf(
  df.cities,
...
# 3.1. Creating and plotting simple features
...
# create these objects into a column
geom <- st_sfc(gm.multipt, gm.linestr, gm.poly)

# our non-geom dataframe
mock_data <- data.frame(objects = c('multipt', 'linestr', 'polyg'))

# appending our geom data to our data_frame, declare it as a sf object
sf_mockdata <- st_sf(mock_data, geometry = geom)

ggplot() +
  geom_sf(data=sf_mockdata)

# 3.2. Downloading external data and plotting: differentiate airport types by color
